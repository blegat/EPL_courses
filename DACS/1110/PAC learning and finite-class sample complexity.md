...
- Instance space, label space, hypothesis class, learner, and unknown data
  distribution.
- True risk $R(h)$ and empirical risk $\\widehat R_S(h)$.
- Accuracy $\\varepsilon$, confidence $1-\\delta$, and sample complexity.
...
complexity $m_\\mathcal H(\\varepsilon,\\delta)$ such that, whenever the labels are generated by
some $h^*\\in\\mathcal H$, the learner returns, with probability at least $1-\\delta$, a
hypothesis satisfying $R(h)\\leq\\varepsilon$ from at least that many i.i.d. samples.
...
$1/\\varepsilon$, $\\log(1/\\delta)$, and an appropriate representation-size parameter.
